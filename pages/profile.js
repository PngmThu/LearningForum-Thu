...
class ProfileShow extends Component {
        ...
        if(this.props.address == 'user'){
            ...
            if(!hasAddress){
                ...
                this.setState({ address: accounts[0], ratings, ratingCount, timesRenting, timesBorrowing, 
                                sumRating, user, productName, loader , isUser, token });
            }
        }
    }
    ...
    static async getInitialProps(props) {
        ...
        //var token = await Profile(address).methods.getToken().call();
        //const token = 3;
        if(address == 'user'){
            var ratings = [];
            var ratingCount = 0;
            ...
            //var token = 1;
            // const accounts = await web3.eth.getAccounts();
            // var token = await Profile(accounts[0]).methods.getToken().call();
        } else {
